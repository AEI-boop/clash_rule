...
function main(config) {
  ...
  config.dns = {
    ...
    "nameserver-policy": {
      ...
      "geosite:geolocation-!cn": trustDnsList,  // 非中国域名使用国外DNS
    },
  ...
  config["proxy-groups"] = [
    ...
    // 【自动选择组】自动测速，选择延迟最低的节点
    {
      ...
      "exclude-filter": "(?i)香港|Hong Kong|HK|hongkong",  // 排除香港节点（正则表达式）
    ...
    // 【YouTube专用组】
    {
      ...
      proxies: [
        "节点选择", "自动选择", "狮城节点", "香港节点", "台湾节点", 
        "日本节点", "美国节点", "韩国节点", "其他节点", "手动切换", "DIRECT"
      ]
    },
    ...
    // 【漏网之鱼组】所有规则都不匹配时使用
    {
      ...
      proxies: [
        "节点选择",  // 默认走代理
        "自动选择", "DIRECT", "香港节点", "台湾节点", "狮城节点", 
        "日本节点", "美国节点", "韩国节点", "其他节点", "手动切换"
      ]
    },
    ...
    // 【香港节点组】自动选择延迟最低的香港节点
    {
      ...
      filter: "(?i)港|HK|hk|Hong Kong|HongKong|hongkong",  // 通过正则表达式筛选香港节点
      type: "url-test",  // 自动测速
    ...
    // 【日本节点组】
    {
      ...
      "include-all": true,
      filter: "(?i)日本|川日|东京|大阪|泉日|埼玉|沪日|深日|JP|Japan",  // 筛选日本节点
      type: "url-test",
      interval: 300,
      tolerance: 50
    },
    ...
    // 【韩国节点组】
    {
      ...
      "include-all": true,
      filter: "(?i)KR|Korea|KOR|首尔|韩|韩",  // 筛选韩国节点
      type: "url-test",
      interval: 300,
      tolerance: 50
    },
    ...
    // 【其他地区节点组】不属于上述地区的节点
    {
      ...
      "include-all": true,
      // 排除上述所有地区，剩下的就是其他地区
      "exclude-filter": "(?i)港|HK|hk|Hong Kong|HongKong|hongkong|台|新北|彰化|TW|Taiwan|新加坡|坡|狮城|SG|Singapore|美|波特兰|达拉斯|俄勒冈|凤凰城|费利蒙|硅谷|拉斯维加斯|洛杉矶|圣何塞|圣克拉拉|西雅图|芝加哥|US|United States|日本|川日|东京|大阪|泉日|埼玉|沪日|深日|JP|Japan|KR|Korea|KOR|首尔|韩|韩",
      type: "url-test",
      interval: 300,
      tolerance: 50
    },
    ...
    // 【全局组】包含所有代理组，方便统一管理
    {
      ...
      proxies: [
        // 列出所有代理组，方便快速切换
        "节点选择", "自动选择", "手动切换", "AI节点", "电报消息", "油管视频", 
        "奈飞视频", "国内媒体", "国外媒体", "谷歌FCM", "微软Bing", "微软云盘", 
        "微软服务", "苹果服务", "游戏平台", "全球直连", "广告拦截", "应用净化", 
        "漏网之鱼", "香港节点", "台湾节点", "狮城节点", "美国节点", "日本节点", 
        "韩国节点", "其他节点"
      ]
    }
  ];
